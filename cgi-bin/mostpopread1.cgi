...
my $numreadings=param("numreadings");
...
my $selectstring= "SELECT COUNT(Feature), Feature FROM ServiceLines WHERE (ServiceDate >= '$startdate' AND ServiceDate <= '$enddate') AND (FType LIKE '\%Prayer\%' OR FType LIKE '\%Reading\%' OR FType LIKE '\%Offering\%') AND $servstring GROUP BY Feature";
my $maxuses=0;
my $dbcursor=$dbh->prepare($selectstring);
$dbcursor->execute();
while (my @dbrow=$dbcursor->fetchrow_array())
{
	if($readings{$dbrow[0]} eq "")
	{
		$readings{$dbrow[0]}=$dbrow[1];
	}
	else
	{
		$readings{$dbrow[0]}=$readings{$dbrow[0]}.";".$dbrow[1];
	}
...
print "<html>\n";
print $myheader;
print "<head><title>$numreadings Most Popular Readings</title></head>\n";
print "<body>\n";
if ($noframes) { TopTable("stats"); }
print "<center>\n";
print "<font size=\"+2\">The $numreadings most popular readings between $fromday/$frommonth/$fromyear and $today/$tomonth/$toyear<br>\n";
print "For @services Services</font><br><br>\n";
print "<table width=\"100%\"border=\"1\" style=\"border-collapse: collapse\">\n";
print "<tr><th align=\"left\" valign=\"top\">Reading</th>\n";
...
for($i=$maxuses;$i>0;$i--)
{
	if($readings{$i} ne "")
	{
		...
		for($j=0;$j<(scalar @_);$j++)
		{
			...
			print "</tr>\n";
			$readingcount++;
			last if ($readingcount == $numreadings);
		}
	}
	last if ($readingcount == $numreadings);
}
...
{
	my $reading=shift;
	my $dbreading=$reading;
	$dbreading =~ s/\'/\\\'/g ;
	$dbcursor=$dbh->prepare("SELECT ServiceDate, Service FROM ServiceLines WHERE Feature='$dbreading' AND ServiceDate >= '$startdate' AND ServiceDate <= '$enddate' AND $servstring ORDER BY ServiceDate DESC, Service ASC");
	$dbcursor->execute();
	print "<select name=\"$reading\">\n";
